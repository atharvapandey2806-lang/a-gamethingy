...
def install_bundled_assets() -> Path:
    ...
    static_src = bundle_dir / "static"
    static_dst = target_dir / "static"
...
class GameBotApp:
    ...
    def build_account_page(self):
        ...
        tk.Button(actions, text="Open Sign In Page", command=lambda: webbrowser.open(f"{SERVER_URL}/login.html")).grid(row=0, column=3, padx=8, ipadx=18, ipady=6)
        tk.Button(actions, text="Open Sign Up Page", command=lambda: webbrowser.open(f"{SERVER_URL}/signup.html")).grid(row=0, column=4, padx=8, ipadx=18, ipady=6)
