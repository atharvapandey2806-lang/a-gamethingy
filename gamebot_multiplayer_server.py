...
try:
    ...
    if not hasattr(jwt, 'encode'):
        raise ImportError('Wrong jwt package installed. Use PyJWT: pip install PyJWT')
except ImportError as exc:
    raise ImportError(
        'PyJWT is required. Run: pip uninstall jwt -y && pip install PyJWT'
    ) from exc
import base64
import hmac
import os
...
def serve_master():
    """Legacy route for old bookmarks."""
    return send_file(os.path.join(BASE_DIR, 'login.html'))
